...
sub BUILD {
    ...
    if (exists $arg_ref->{state}) {
		...
		if ( $state->isa('ANTLR::Runtime::RecognizerSharedState') ) {
			$self->following($state->following);
	        $self->_fsp($state->_fsp);
	        $self->error_recovery($state->error_recovery);
	        $self->last_error_index($state->last_error_index);
	        $self->failed($state->failed);
			$self->syntax_errors($state->syntax_errors);
			$self->backtracking($state->backtracking);
			$self->rule_memo($state->rule_memo) if (defined $state->rule_memo);
			$self->token($state->token);
			$self->token_start_char_index($state->token_start_char_index);
	        $self->token_start_char_position_in_line($state->token_start_char_position_in_line);
