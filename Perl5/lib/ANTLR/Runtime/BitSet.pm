...
sub BUILD {
    ...
    if (!%$args) {  ## no critic (ControlStructures::ProhibitCascadingIfElse)
        ...
        foreach my $h (split //xms, reverse $number) {
            ...
            $bits .= substr(unpack('B*', pack('h', hex $h)), 4);
        }
    }
    elsif (exists $args->{''}) {
...
sub of {
	...
	if ( 2 == @_ ) {
	    ...
	    $bs = ANTLR::Runtime::BitSet->new({ size => $el + 1 });
	    $bs->add($el);
	}
	elsif ( 3 == @_ ) {
		my ($class, $a, $b) = @_;
		
		$bs = ANTLR::Runtime::BitSet->new({ size => max($a,$b) + 1 });
		$bs->add($a);
		$bs->add($b);
	}
	elsif ( 4 == @_ ) {
		...
		dlock( my $usage = 'BitSet of(int el) | BitSet of(int a, int b) | BitSet of(int a, int b, int c) | BitSet of(int a, int b, int c, int d)' );
		croack $usage;
	}
...
sub grow_to_include : method {
    ...
    if ($bit > length $self->bits) {
        $self->bits($self->bits . '0' x ($bit - (length $self->bits) + 1));
    }
...
sub length_in_long_words : method {
    my ($self) = @_;
    return int($self->num_bits() / $self->BITS);
}
